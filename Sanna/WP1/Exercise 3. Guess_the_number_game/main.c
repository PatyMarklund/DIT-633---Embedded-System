...
#define MAX_NUMBER 5                     // wherever it says MAX_NUMBER it translates to 5 in the code. Used for max num of guesses
...
int guessNumberGame() {
    ...
    do {                                                            // executes the body atleast once and as long as the while condition is true
        printf("\nGuess the number:");
        ...
        if (read_number != 1) {                                     // Will only execute this if statement if the input for guessed number was unsuccessful (0). 
        ...
        else if (guessedNumber >= 1 && guessedNumber <= 100) {      // will only execute this if statement if the input for guessed number is between 1-100.

            if (guessedNumber == randomNumber) {                    // execute if the user guesses the correct number
            ...
            else if (guessedNumber < randomNumber) {                // execute if the users guessed a lower number than the random number 
                numberOfGuesses = numberOfGuesses + 1;              // adds a count on number of guesses
                printf("Your guess is to low!\n");
            }                                                     
            else if (guessedNumber > randomNumber) {                // execute if the user guessed a higher number than the random number 
                numberOfGuesses = numberOfGuesses + 1;              // adds a count on number of guesses
    ...
    if (numberOfGuesses >= MAX_NUMBER) {                            // will only execute this if statement if the termination depends on max guesses being reached.
...
int exitOrContinue() {
    ...
    if (exitNumber == 1) {                                               // execute if the user types 1
        exit(0);                                                         //terminate the program
    }
    else if (read_exit != 1) {                                           // If this value is 0, something other than a number has been provided
    ...
    } else guessNumberGame();                                           // calls the function to play the game again
