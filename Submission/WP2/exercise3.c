...
void write_new_file(PERSON *inrecord)
{
    ...
    filePtr = fopen(FILE_NAME, "wb");
    if (!filePtr)  // Check if the file is existant
    {
...
//Gives an opportunity to add in one new person to the Person structure
...
// Search for a Person using the firstname
...
// Search for a Person using the family name
...
void printfile()
{
    ...
    filePtr = fopen(FILE_NAME, "rb"); // rb = Open an existing file for reading (binary)
    if (filePtr == NULL)     // If the file is not existant
    {
...
unsigned int enterChoice(void)
{
    ...
    return menuChoice;   // Return the choice to the main
}