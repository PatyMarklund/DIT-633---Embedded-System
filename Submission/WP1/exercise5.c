...
{
    for (int i = 0; i < MAX; i++)
    {                                      // Loop throught the array tab which has a size of the defined MAX number
...
void count_frequency(int *tab, int *freq)
{
    ...
    { // Loops through the possible numbers between 0 – MAXNUMBER
        for (int j = 0; j <= MAX; j++)
        { // Loop throught the array tab which has a size of the defined MAX number
            if (tab[j] == i)
            {            // Compare the index number from the first loop with the int value of the tab array
...
void draw_histogram(int *freq)
{
    ...
    for (int i = 0; i <= MAXNUMBER; i++)
    { // Loops through the array frequency MAXNUMBER
        if (!freq[i] == 0)
        {                      // Check if the value in that current index is not equal 0.
            printf("%d\t", i); // Prints the index number
            for (int j = 0; j < freq[i]; j++)
            {                // Loops through the freq[i]. So it will count for each number stored in the current index it will print a character
